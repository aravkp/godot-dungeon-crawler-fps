...
const CHAIN := [
	["Hips", "Spine1", 0.50, 12.0, 0.0, 0.0],
	["Spine1", "Neck", 0.42, 10.0, 25.0, 20.0],
	["Head", "HeadTop_End", 0.34, 4.0, 30.0, 30.0],
	["LeftArm", "LeftForeArm", 0.28, 2.5, 75.0, 40.0],
	["LeftForeArm", "LeftHand", 0.18, 2.0, 75.0, 20.0],
	["RightArm", "RightForeArm", 0.28, 2.5, 75.0, 40.0],
	["RightForeArm", "RightHand", 0.18, 2.0, 75.0, 20.0],
	["LeftUpLeg", "LeftLeg", 0.22, 5.0, 55.0, 25.0],
	["LeftLeg", "LeftFoot", 0.17, 4.0, 65.0, 10.0],
	["RightUpLeg", "RightLeg", 0.22, 5.0, 55.0, 25.0],
	["RightLeg", "RightFoot", 0.17, 4.0, 65.0, 10.0],
]
...
## `layer` 0 by default: a corpse is scenery, nothing should target it. The mask
## is what still makes it land on the floor.
## Returns null if the rig has too few of the expected bones to be worth it.
static func build(skel: Skeleton3D, layer: int = 0, mask: int = 1) -> PhysicalBoneSimulator3D:
...
## Go limp. `impulse` is world-space; it is applied to whichever body is nearest
## `at`, so a headshot snaps the head back and a leg hit sweeps the legs.
static func start(sim: PhysicalBoneSimulator3D, at: Vector3 = Vector3.INF,
		impulse: Vector3 = Vector3.ZERO) -> void:
	if sim == null or not is_instance_valid(sim):
		return
