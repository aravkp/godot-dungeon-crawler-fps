...
func handle_set_mesh(params):
	...
	# Dimensions arrive nested under "mesh_params" (see set_mesh in object_tools.py),
	# but fall back to the top level so either shape of payload works.
	var mesh_params = params.get("mesh_params", params)
	...
	match mesh_type.to_upper():
		"CAPSULEMESH":
			...
			if mesh_params.has("radius"):
				mesh.radius = float(mesh_params.radius)
			if mesh_params.has("height"):
				mesh.height = float(mesh_params.height)
		"BOXMESH":
			mesh = BoxMesh.new()
			if mesh_params.has("size"):
				var size = mesh_params.size
		...
		"CYLINDERMESH":
			...
			if mesh_params.has("radius"):
				mesh.top_radius = float(mesh_params.radius)
				mesh.bottom_radius = float(mesh_params.radius)
			if mesh_params.has("height"):
				mesh.height = float(mesh_params.height)
		"PLANEMESH":
			mesh = PlaneMesh.new()
